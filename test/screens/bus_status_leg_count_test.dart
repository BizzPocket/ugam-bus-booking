...
/// Regression for AS-2: the STATUS tab's tally (`n/total` text + the `%`
/// fill bar) used to fold raw [SeatAssignment] entries per bus via
/// `assignments.values.fold(...)`, so a seat shared by two opposite
/// one-way riders (GO-only + RET-only on the same physical berth) counted
/// TWICE — the tally could read past capacity, e.g. "40/37", and the ratio
/// clamp masked it as "100%". `BusStatusScreen` has no test seam (it reads
/// a live `TourController` singleton — see `handler_bus_chart_screen_test.dart`),
/// so this pins the leg-aware value `Tour.occupiedBerthsFor` must supply and
/// that the screen now displays, mirroring the fixture pattern in
/// `test/screens/charts_leg_count_test.dart` (AS-1) and
/// `test/models/tour_leg_occupancy_test.dart`.
...
void main() {
  test('bus-status tally uses the leg-aware count so the ratio never exceeds 100%', () {
    ...
    expect(assigned, 3);
    expect(assigned, lessThan(rawFold));
