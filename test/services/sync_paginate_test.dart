...
void main() {
  ...
  test('stops after a short page and returns every row in order, no drops or dupes', () async {
    final calls = <int>[];
    final rows = await paginateRows<int>(fakeSource(2300, calls: calls), pageSize: 1000);
    // Checks actual content/order, not just length — a boundary drop or
    // duplicate at a page seam (e.g. row 999 or 1000 missing/repeated) would
    // fail this equality even though rows.length could still coincidentally
    // match.
    expect(rows, List.generate(2300, (i) => i));
  ...
  test('exact-multiple boundary fetches one extra empty page then stops', () async {
    ...
    expect(rows, List.generate(2000, (i) => i));
  ...
  test('single short page', () async {
    ...
    expect(rows, List.generate(42, (i) => i));
    expect(calls, [0]);
  });
}
